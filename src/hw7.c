...
matrix_sf* mult_mats_sf(const matrix_sf *a, const matrix_sf *b) {
    ...
    int rows = a->num_rows;
    int cols = a->num_cols;
    int cols2 = b->num_cols;

    matrix_sf *res = malloc(sizeof(matrix_sf) + rows * cols2 * sizeof(int));
    res->name = '?';
    res->num_rows = rows;
    res->num_cols = cols2;
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols2; j++) {
            ...
            for (int k = 0; k < cols; k++) {
                sum = sum + a->values[i*cols + k] * b->values[k*cols2 + j];
            }
            res->values[i*cols2 + j] = sum;
        }
    }
...
matrix_sf* transpose_mat_sf(const matrix_sf *m) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            res->values[j*rows + i] = m->values[i*cols + j];
        }
    }
